...
// AES implementation for CPU (based on tiny-AES-c)
...
#define Nk 4        // Number of 32-bit words in the key (4, 6, or 8 for AES-128, AES-192, or AES-256)
#define Nr 10       // Number of rounds (10, 12, or 14 for AES-128, AES-192, or AES-256)
...
// AES Cipher - the main encryption function
static void Cipher(uint8_t* state, const uint8_t* RoundKey) {
    ...
    // The last round is special - no MixColumns
    SubBytes(state);
    ShiftRows(state);
...
void AES_CTR_xcrypt_buffer(struct AES_ctx* ctx, uint8_t* buf, size_t length) {
    ...
    for (i = 0, bi = 16; i < length; ++i, ++bi) {
        if (bi == 16) { // Need to generate new keystream block
...
// Main AES-CTR encryption/decryption function
void aes_ctr_encrypt(const uint8_t* plaintext, uint8_t* ciphertext,
                     const uint8_t* key, const uint8_t* iv, size_t size) {
    struct AES_ctx ctx;
    ...
    AES_CTR_xcrypt_buffer(&ctx, ciphertext, size);
}
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i <= iterations; i++) {
        ...
        aes_ctr_encrypt(plaintext, ciphertext, key, iv, fileSize);
        ...
        aes_ctr_encrypt(ciphertext, decrypted, key, iv, fileSize);
